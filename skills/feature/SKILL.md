...
5. **Review** the design: run the [critique gate](${CLAUDE_PLUGIN_ROOT}/docs/critique-gate.md) with [critique-design](${CLAUDE_PLUGIN_ROOT}/docs/critique/critique-design.md). Do not call ExitPlanMode until it passes. For architecturally complex designs, run a second pass before proceeding.
...
5. **Critique the plan**: run the [critique gate](${CLAUDE_PLUGIN_ROOT}/docs/critique-gate.md) with [critique-design](${CLAUDE_PLUGIN_ROOT}/docs/critique/critique-design.md). Plan-level critique catches structural bugs — wrong ordering, missing error paths, inconsistent interfaces between steps — that are expensive to discover during implementation. Do not skip or compress this step. Each structural fix can introduce new inconsistencies that a single pass won't catch, so re-run after applying fixes until it passes.
...
Before the exit gate, wait for any outstanding critique subagents to complete. Integrate findings: address Critical and Significant findings before proceeding (the gate's pass bar), note Minor findings for Phase 5 clean-up.
