...
**Subagent nesting**: Subagents can spawn their own subagents, but by policy data-gathering Task subagents must not spawn further agents (see project CLAUDE.md). Critique needs a subagent for context separation, so spawn it from the orchestrating session — this lead, or an exempt Planning/implementer teammate — never from a data-gathering Task subagent. Skills that compose other skills must account for this: invoke phronithm:critique and other Agent-dependent skills from the orchestrating session.

**Running as a teammate**: A teammate executing a batched work item is a full orchestration session and is exempt from the no-nesting policy — it spawns the phronithm:critique and phronithm:review agents these phases require (critique in Phases 2–4, review and critique in Phase 5) exactly as the lead would, in their own separate contexts. No inline fallback or confidence reduction is needed. The only residual fallback is the degenerate case of this skill being driven from an ordinary data-gathering subagent (not a teammate): there the no-nesting policy forbids spawning, so run the evaluative steps inline, note the reduced confidence from missing context separation, and flag for the orchestrating session or a later review pass.
...
- **Session record missing team data.** Phase 5 postmortem runs without team activity visible in context. Two sub-cases: (a) *Context available*: at the start of Phase 5, output a team activity summary — teammates spawned, phases handled, interaction pattern — before further compression can remove it. (b) *Context already fully compressed before Phase 5*: team data is unrecoverable; note the data gap in `workflow-quality` and omit the `agent-team` block from the session record.
...
5. **Review** the design by spawning a Sonnet Agent (`model: "sonnet"`) to invoke `/phronithm:critique` with [critique-design](${CLAUDE_PLUGIN_ROOT}/docs/critique/critique-design.md). Critique must run in a separate context from the design author — never invoke it directly in this session. Do not call ExitPlanMode until at least one critique round returns no critical findings. For architecturally complex designs, run a second critique pass before proceeding.
...
5. **Critique the plan** by spawning a Sonnet Agent (`model: "sonnet"`) to invoke `/phronithm:critique` with [critique-design](${CLAUDE_PLUGIN_ROOT}/docs/critique/critique-design.md). Plan-level critique catches structural bugs — wrong ordering, missing error paths, inconsistent interfaces between steps — that are expensive to discover during implementation. Critique must run in a separate context from the plan author — never invoke it directly in this session. Do not skip or compress this step. When the first critique round returns Critical or Significant findings, run a second pass after applying fixes. Each structural fix can introduce new inconsistencies that a single pass won't catch. Stop when no Critical or Significant findings remain.
...
7. **Append a handoff footer** to the plan document:
   ...
   complete Phase 5 (Review, including postmortem) as defined in the phronithm:feature skill.
...
1. **Review** the implementation by running the phronithm:review skill — scope is the full feature diff, all applicable lenses, static-analysis pre-scan required (not optional). Collect per-step critique findings from Phase 4 into the review scope. Additionally, run a final [phronithm:critique](${CLAUDE_PLUGIN_ROOT}/docs/critique.md) + [critique-code](${CLAUDE_PLUGIN_ROOT}/docs/critique/critique-code.md) of the full diff to catch issues that emerge only at feature scope.
...
Exit criteria: Code is reviewed, tested, documented, and ready to merge. Commit history is tidy.
...
- **Serialisation-readiness**: Would this phase's produces be sufficient for the next phase to operate without session context? Rate `sufficient` / `partial` / `insufficient`. If partial or insufficient, note what is missing. This is aggregated in the Phase 5 postmortem.
