...
7. **Critique the architecture brief** — this step catches structural bugs that are expensive to discover later. Do not skip or compress it. Run the [critique gate](${CLAUDE_PLUGIN_ROOT}/docs/critique-gate.md) on the architecture brief with `critique-design`, applying its default iteration policy. Present critical and significant findings to the requester and confirm the proposed fixes before implementing them — this skill's multi-session, higher-stakes structure warrants requester sign-off on fixes that the single-session `phronithm:feature` doesn't require. Minor findings may be fixed without review.
